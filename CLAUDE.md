...
| `lib/muskelgruppen.php` | `MG_SORT_JOIN` und `MG_SORT_ORDER` — die **eine** Stelle, an der steht, in welcher Reihenfolge ein Übungs*katalog* erscheint (§6.2, Fallstrick 9). Zwei SQL-Bausteine, keine Funktionen; der Alias der Übungstabelle muss `e` heißen. Dazu seit `1.5.2` `MG_ZUORDNUNG_JOIN`/`MG_ZUORDNUNG_ORDER`: die Reihenfolge der Gruppen **innerhalb** einer Übung (primär, dann dieselbe Hauptgruppe, dann wie auf `admin_muscle_groups.php`); Aliase `emg`/`mg`. **Wer eine neue Anzeigestelle für Muskelgruppen baut, sortiert damit** — sonst steht dieselbe Übung an zwei Stellen verschieden da |
