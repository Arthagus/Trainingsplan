...
| Version | Was |
|---|---|
| `1.5.1` | *(Sprung auf die neue Basisnummer auf Ansage des Benutzers am 2026-09-17: „weil es eine neue Funktion ist, gehe auf 1.5.1“. Die Änderungen standen zuvor als `1.4.13` im Arbeitsstand, die nie gebaut wurde; `1.5.0` gibt es nicht.)* **Ausdauerzeit nur noch in Minuten und Sekunden.** Wunsch des Benutzers vom 2026-09-17: Ab einer Stunde zeigten `dauer_mmss()`/`dauerMMSS()` ein drittes Feld (`1:13:25`), das man als Stunden und Minuten liest. In der Trainingsansicht und im Eingabefeld jetzt immer `m:ss` mit fortlaufenden Minuten (`73:25`, Obergrenze `360:00`); `h:mm:ss` wird als Eingabe nicht mehr angenommen. Der Verlauf zeigt ab einer Stunde weiterhin `h:mm:ss` (`dauer_hms()`, nur PHP) — Nachtrag des Benutzers am selben Tag: „eingetippt 73:40, im Verlauf 1:13:40“. Gespeicherte Sekunden unverändert, keine Migration. PHP- und JS-Hälfte unter PHP 8.5 bzw. Node gegeneinander ausgeführt, zeichengleich; Gegenprobe gegen `HEAD` zeigte die alte Stundenform <br>**Dazu: Einheiten nachträglich bearbeiten** (Wunsch vom 2026-09-17 — ein im Studio erhöhtes Gewicht war nicht eingetragen und trug sich über die Vorbelegung fort). Knopf „Einheit bearbeiten“ im Verlauf mit Sicherheitsabfrage, Satzliste je protokollierter Übung, neue Aktion `api/log.php → correct` (Fallstrick 35). Kein Schema. Lokal per `curl` geprüft (Kraft, Unterstützung, Ausdauer, fremde Zeile, laufende Einheit, fehlende/ungültige Liste, Altzeile, Wirkung auf „zuletzt …“ und Verlauf), dabei ein Fehler gefunden und behoben: `done` wurde ohne `CAST` nie gesetzt. Gegenproben: laufende-Einheit-Sperre und Eigentümerprüfung entfernt — beide schlugen an. `history.js` unter jsdom gegen die echte Seite ausgeführt (16 Fälle), drei Gegenproben kippten genau die erwarteten Fälle |
| `1.4.12` | **Bildausschnitt „ganzes Bild“.** Wunsch des Benutzers vom 2026-09-15: Neben *links / Mitte / rechts* eine vierte Option, bei der ein breites Bild im quadratischen Vorschaurahmen verkleinert statt beschnitten wird; die große Ansicht bleibt wie bisher. Umgesetzt als Wert `ganz` in `ZUSCHNITT`, Klasse `bild-ganz` (PHP und `vorschlagMarkup()`), im Stylesheet `object-fit: contain`. Keine Migration. Der Selektor lautet `img.bild-ganz`, weil `.vorschlag-bild` und `.position-bild` weiter unten `cover` setzen und eine bloße Klassenregel dort verloren hätte |
| `1.4.11` | **Übungen, bei denen weniger Gewicht besser ist.** Frage des Benutzers vom 2026-09-15: An einer Klimmzug-Maschine mit Unterstützung stellt man die abgenommene Last ein, bei 0 kg sind es normale Klimmzüge. Die App wertete an vier Stellen „mehr ist besser“ aus: Leitgewicht (schwerster Satz), Bestwert (`MAX`), Volumen/1RM (Wdh × kg) und die Kurve. Neu ist `exercises.gewicht_wirkung` (`last`/`unterstuetzung`), im Formular „Gewicht bedeutet“. **Bewusst keine dritte Erfassungsart**, weil die den Tausch trennte — unterstützte Klimmzüge sollen gegen gewöhnliche tauschbar bleiben; daraus folgt, dass die Wirkung der *ausgeführten* Übung zählt. Entscheidungen des Benutzers: Volumen und 1RM **ausblenden** statt mit einem Körpergewicht zu rechnen, Kurve **spiegeln** (oben = besser). Beim Prüfen fiel auf, dass `plan_positionen()` die neue Spalte zwar abfragte, aber nicht zurückgab — die Karte trug für jede Übung `last`. **Lokal per `curl` gegen PHP 8.5 geprüft**, mit drei Gegenproben (Planübung statt ausgeführter Übung / MIN-Zweig entfernt / Neuberechnung beim Umstellen entfernt), die jeweils genau den erwarteten Fall kippten; die Formularumschaltung unter Node ausgeführt, samt Gegenprobe. Einzelheiten in Fallstrick 34 |
| `1.4.7` | **Die Sortierpfeile der Muskelgruppen — und die Wirkung ihrer Reihenfolge auf die Übungslisten.** **Erstens:** *„Bei den Untergruppen habe ich hoch und runter Pfeile, aber die funktionieren nicht"* (2026-09-03, aus dem Live-System). `admin_muscle_groups.js` rief `insertBefore()` immer gegen die oberste Liste auf, während eine Untergruppe in ihrem eigenen `<ul>` innerhalb des Halters steht — der Aufruf warf `NotFoundError`, und zwar in einem `async`-Handler, den niemand abfängt: **kein Fehler auf dem Bildschirm, der Knopf tut schlicht nichts.** Beim Messen kam die zweite Hälfte dazu: Auch **Hauptgruppen** mit Untergruppen waren kaputt, nur unsichtbar. `previousElementSibling` zeigte dort auf den Halter statt auf die Nachbargruppe; ein Tipp nach unten schob die eigenen Kinder über die Hauptgruppe und schickte eine Tiefensuche an den Server, in der eine Untergruppe **vor** ihrer Hauptgruppe steht — Antwort `ok:true`, und weil die Seite den Baum beim nächsten Aufbau neu gruppiert, sah man nur, dass sich nichts bewegt hatte. Behoben über `zeile.parentElement`, einen übersprungenen Halter und einen Block, der die Hauptgruppe samt ihren Kindern bewegt; dazu Randpfeile gesperrt, alle Pfeile während des Speicherns gesperrt (Fallstrick 28) und ein Zurücknehmen statt Neuladen im Fehlerfall — ein Neuladen würfe nebenbei jede unbestätigte Namensänderung weg. **Im Browser reproduziert** und nach dem Fix gegengeprüft mit drei gezielten Rücknahmen (Halter nicht überspringen / Halter nicht mitnehmen / wieder gegen die oberste Liste einfügen): Jedes Mal fiel genau das Erwartete durch und nur das. **Zweitens, im selben Zug auf Ansage:** *„Überall, wo Übungen aufgelistet werden, sollte die Liste der Sortierung der Muskelgruppen entsprechen."* Übungsverwaltung und Übungsauswahl folgten dem Alphabet — die mühsam sortierte Gruppenliste hatte außerhalb ihrer eigenen Seite keine Wirkung. Die Regel steht jetzt **einmal**, als `MG_SORT_JOIN`/`MG_SORT_ORDER` in der neuen `lib/muskelgruppen.php`. Drei Entscheidungen darin, alle am Bestand nachgemessen: Sortiert wird nach der **Primärgruppe** — sonst hinge der Platz einer Übung an einer zufälligen Nebengruppe. **`LEFT JOIN` ist Pflicht**, weil eine Ausdauerübung seit `1.4.1` gar keine Muskelgruppe hat; die Gegenprobe mit innerem Verbund ließ das Laufband **lautlos** aus beiden Listen verschwinden. Und geordnet wird **erst nach der Wurzel, dann innerhalb**, weil eine neu angelegte Untergruppe `MAX + 10` bekommt und damit global ganz hinten hängt, während die Seite sie längst unter ihrer Hauptgruppe zeigt. Nicht angetastet: Planpositionen (dort zählt die Planreihenfolge), Tauschvorschläge (folgen `mg.sort_order` ohnehin schon) und der Verlauf (zuletzt trainiert zuerst) |
| `1.4.6` | **Kein `autofocus` mehr — der Passwortmanager am Handy.** Gemeldet am 2026-09-02: Auf einem Pixel 10 bot Proton Pass in Firefox die Zugangsdaten auf `login.php` nicht an, und zwar ohne dass auch nur sein Symbol über der Tastatur erschien. Die Ursache ist keine Eigenheit des Managers: **Firefox auf Android meldet ein Feld, das beim Laden bereits den Fokus hat, nicht an Androids Autofill-Dienst.** Am Desktop fällt das nie auf, weil der Manager dort eine Erweiterung ist und das DOM liest, statt am Fokus zu hängen — dieselbe Sorte Trugschluss wie „auf meinem Gerät ruhig" bei Fallstrick 19d und 19g. Seither gibt es im ganzen Projekt **kein** `autofocus` mehr; wo ein Feld den Fokus tragen soll, wird er in JS gesetzt und nur am Zeigegerät (`matchMedia('(pointer: coarse)')`), damit ein Notebook mit Touchscreen und Maus ihn behält. **Am Gerät bestätigt.** In Chrome lag es dagegen nicht an der App: Chrome auf Android reicht Webseiten ab Werk nur an den Google Passwortmanager weiter — nach dem Umstellen des Autofill-Dienstes schlägt Proton auch dort vor. Die ursprüngliche Meldung nannte Firefox, meinte aber teils die mit Chrome angelegte Startbildschirm-Verknüpfung; dass der `autofocus`-Ausbau die Firefox-Seite gelöst hat, ist damit plausibel, aber nicht bewiesen — richtig ist der Ausbau unabhängig davon |
| `1.4.5` | **Das verschwundene Häkchen, zweiter Anlauf — die Warteschlange löschte, was sie noch nicht verschickt hatte.** Am 2026-08-31 stand Neles letzte Einheit auf „7/8“, obwohl sie alles abgehakt hatte. Der Fix aus `1.4.3` (der Parameter `bestaetigt`) war richtig, deckte aber nur die eine Hälfte ab: Er verhindert, dass eine **fachliche Ablehnung** ein Häkchen sichtbar wegnimmt. Die zweite Hälfte liegt im Aufräumen nach dem `await`. `abarbeiten()` nahm den erledigten Eintrag mit einem blanken `schlange.entfernen(peId)` heraus — und traf damit auch einen Eintrag, der während des Aufrufs neu gesetzt worden war. Genau das passiert, wenn man die letzte Satzzeile ausfüllt (nach 800 ms geht `done: false` mit den Sätzen raus) und kurz darauf auf *Erledigt* tippt, während dieser Aufruf noch unterwegs ist: Der zweite Eintrag trägt `done: true`, wird von der Antwort des ersten weggeräumt und **nie verschickt**. Auf dem Bildschirm sitzt das Häkchen, in der Datenbank steht die Zeile mit Sätzen und `done = 0` — dieselbe Signatur wie beim ersten Fall, nur ohne jedes sichtbare Zeichen. Im Studio-Netz ist das Zeitfenster Sekunden groß. **Behoben** durch `eintragAbschliessen()`: entfernt wird nur, was noch **derselbe** Eintrag ist, verglichen am `ts`; ein neuerer bleibt liegen und geht im nächsten Durchgang raus. Im Fehlerzweig kommt eine zweite Folge dazu — eine überholte Ablehnung wird **nicht mehr gemeldet**, weil die Nutzlast die Zeile vollständig beschreibt und der neue Eintrag den alten samt seinem Fehler ersetzt. **Die allgemeine Lehre steht in Fallstrick 13:** Wer nach einem `await` aufräumt, räumt einen Zustand auf, der sich inzwischen geändert haben kann — entfernt wird nie „der Eintrag zu dieser ID“, sondern „genau dieser Eintrag“. **Nachgewiesen am ausgeführten Code** (die echten Funktionen aus `index.js` und `assets/app.js` unter Node, mit einem Aufruf, der auf Kommando antwortet), samt Gegenprobe: Ohne die neue Bedingung fällt **genau** der Rennfall um und der normale Ablauf nicht. **Dazu die Sicherheitsstufe darunter, auf Ansage des Benutzers vom 2026-09-01:** Beim Beenden einer Einheit bekommt jede Position mit **mindestens einem Satz** `done = 1`; offen bleibt nur, wozu gar nichts eingetragen wurde. Damit fallen „protokolliert“ und „erledigt“ (Fallstrick 18) genau an dem Punkt zusammen, an dem die Frage „bin ich fertig?“ beantwortet ist — **während** der Einheit bleibt die Trennung, sonst färbte sich die Karte mit dem ersten Satz blau und die Abschlussfrage käme mitten in der Übung. Serverseitig in `einheit_beenden()`, weil beide Ursachen im Browser saßen und ein Netz, das durch die kaputte Ebene läuft, keines ist. **Der Preis steht dazu:** Eine bewusst abgebrochene Übung gilt danach als erledigt — wer sie offen halten will, löscht ihre Sätze vor dem Beenden. Nachgemessen gegen eine Testdatenbank (abgehakt-mit-Sätzen bleibt, offen-mit-Sätzen wird nachgetragen, offen-ohne-Sätze bleibt offen) und über HTTP; die Gegenprobe ohne die `EXISTS`-Bedingung hakt zusätzlich **genau** die Position ohne Sätze ab und sonst nichts. |
| `1.4.4` | **Zwei Zählfehler und ein verrutschtes Häkchen — die Nummer trägt beides.** Ein erstes Paket unter dieser Nummer wurde gebaut, aber nie eingespielt; der Benutzer hat sie am 2026-08-30 ausdrücklich freigegeben („meine letzte aktive Nummer, die gerade live ist, ist 1.4.3"). Nach der Regel *„ein Paket, das den Rechner nie verlassen hat, gibt seine Nummer wieder frei"* wurde unter `1.4.4` neu gebaut, statt eine Lücke zu lassen. **Erstens:** **„Erledigt" stand in der Mitte statt rechts.** Regression aus `1.4.3`, vom Benutzer am selben Tag aus dem Studio gemeldet. Mit dem einfachen Modus fiel das Gewichtsfeld aus `.position-aktionen` weg — und damit war „Erledigt" das zweite Kind eines Rasters aus drei Spalten. Ein Grid füllt seine Spalten der Reihe nach: Das Häkchen landete in der **mittleren**, und `justify-self: end` richtete es brav am rechten Rand *dieser* Spalte aus. Das sieht aus wie ein kaputtes Alignment und ist eine verrutschte Platzierung. **Die Lehre ist allgemeiner als der Fall:** Wo eine Spalte absichtlich leer bleibt, gehört die Zuweisung ausdrücklich hin und darf nicht der automatischen Platzierung überlassen werden. Und methodisch: Der Umbau war am HTML geprüft — dass die Zeile *aussieht* wie vorher, sagt das HTML nicht. Diesmal wurde gerendert (Firefox headless, 390 px und 340 px, dazu der Fehlerfall mit sichtbarem Wiederholen-Knopf) **Zweitens:** **10/8 im Verlauf — der zweite Zählfehler, und der naheliegende Griff war falsch.** Nach der Reparatur der verlorenen Häkchen fielen dem Benutzer ältere Einheiten mit `10/8` auf. `einheiten_verlauf()` zählt „x“ über die Protokollzeilen der Einheit, „n“ aber über die **heutigen** Planpositionen; wird eine Übung später aus dem Plan genommen, bleibt ihre Zeile (`ON DELETE SET NULL`, §4.1) und zählt in „x“ weiter, während „n“ schrumpft. **Mein erster Vorschlag war, die Waisenzeilen zu „n“ zu addieren — das hätte aus „10/8“ ein „10/15“ gemacht**, weil die acht heutigen Positionen bereits welche enthalten, die es damals nicht gab. Der Fehler wäre nicht aufgefallen, wenn wir nicht am echten Bestand gerechnet hätten: Einheit 22 hatte zehn Zeilen, sieben Waisen und acht heutige Positionen, von denen drei erst später dazukamen. Genommen wird jetzt das **Größere** aus Protokollzeilen und Planpositionen — nie mehr `x > n`, und „6/8“ für ausgelassene Übungen bleibt. **Der Umweg über eine Sicherung war der Schlüssel:** Der Benutzer hat den Live-Bestand als Datenbanksicherung übergeben, hier ausgewertet und korrigiert, dann wieder eingespielt — der von `CLAUDE.md` vorgesehene Weg für Datentransfer. Erst dabei zeigte sich das Ausmaß: **41 Waisenzeilen** bei einem Benutzer, keine einzige beim anderen, und die verlorenen Häkchen waren mit sechs Zeilen in zwei Einheiten seltener als befürchtet |
| `1.4.3` | **Ein verschwindendes Häkchen behoben, der einfache Modus ausgebaut, der Stairmaster dazu.** **Der Fehler zuerst**, weil er aus dem Betrieb kam: *„Ich hatte beim Nutzen der App im Studio jetzt zwei Mal den Fall, dass eine bereits abgehakte Übung auf einmal nicht mehr abgehakt war."* Der Benutzer wollte eine Fehlbedienung nicht ausschließen — es war keine. In `abhaken()` stand seit `1.0.8` `vorher = !erledigt`: die Annahme, jeder Aufruf schalte das Häkchen um. Für den Klick aufs Häkchen stimmt sie, für `satzSpeichernJetzt()` nicht, denn das Speichern von Sätzen lässt das Häkchen unberührt. Bei einer abgehakten Übung entstand damit ein Warteschlangen-Eintrag mit `vorher: false`, und die 4xx-Behandlung in `abarbeiten()` setzte das Häkchen brav darauf zurück. **Die Verkleidung war, dass beides für sich richtig aussieht:** Der Rückfall auf `vorher` ist die korrekte Antwort auf eine abgelehnte Eingabe, und `!erledigt` ist der korrekte Vorherzustand — nur eben für einen anderen Aufrufer. Seither ist `bestaetigt` ein eigener Parameter **ohne Vorgabewert**; ein Vorgabewert wäre die Falle selbst gewesen. **Und der Anzeigefehler konnte in einen echten kippen:** Ist das Häkchen erst weg, sind die Satzfelder wieder bedienbar, und der nächste Satz-Speicher schickt `done: false` an den Server. Serverseitig war dagegen nichts kaputt — nachgemessen, dass Schreibvorgänge an anderen Positionen eine abgehakte Zeile nicht anfassen. **Der einfache Modus** ist im selben Zug ausgebaut. Zwei Punkte an einem Tag. **Den einfachen Modus** gab es seit `1.1.0` neben der satzgenauen Erfassung, umschaltbar über `users.expert_mode`. Der Benutzer hat die Folgen vorab abklären lassen und dann entschieden: *„aktuell benutzt niemand den einfachen Modus, daher halte ich ihn mittlerweile für unnötig."* Dazu ein Argument, das in der Abklärung nicht vorkam: Der Platz zwischen *Tauschen* und *Erledigt*, den im einfachen Modus das Gewichtsfeld belegte, wird damit frei für etwas Späteres — das Raster bleibt deshalb dreispaltig mit leerer Mitte. **262 Zeilen weniger.** Die Abklärung hatte drei Dinge vorab geprüft, und alle drei hielten: Der **Verlauf** brauchte keine Änderung, weil `$mitSaetzen` je Einheit gerechnet wird und nicht je Benutzer — alte Einheiten stehen unverändert mit `Übung | Gewicht` da. Das **Abhaken ohne Werte** funktioniert weiterhin, die Zeile entsteht mit `weight = NULL`. Und `gewicht_pruefen()` durfte **nicht** mitgelöscht werden: Sie trägt genau diesen Fall und zusätzlich die wartenden Einträge von vor dem Umbau. Nachgemessen mit einer Gegenprobe — ohne sie kommt ein solcher Eintrag mit `weight = NULL` an, das Gewicht ist lautlos weg. Genau deshalb konnte der Warteschlangen-Schlüssel auf `-v3` bleiben, statt beim Rollout die Eingaben von jedem zu verwerfen, der gerade trainiert. **Der Stairmaster** kostete, was der Doc-Block in `lib/geraete.php` seit `1.0.x` zusagt: eine Zeile in `GERAETE` und ein `<symbol>`. **Dabei eine Altlast beseitigt:** Die *Anzahl* der Geräte stand als Zahl im Fließtext — „sieben" an vier Stellen, ab `1.4.0` falsch, dann „zehn", und mit dem Stairmaster sofort wieder falsch. Sie steht jetzt nirgends mehr. Dieselbe Sorte wie „alle sieben Seiten-Skripte" beim Service Worker, die am selben Tag beim `/init`-Durchgang aufgefallen war: **Eine Zahl, die man von Hand nachziehen muss, zieht niemand nach.** Zugleich die Antwort auf die Frage des Benutzers, ob sich Geräte selbst pflegen ließen: Sie können es nicht, und der Grund ist nicht die Liste, sondern das Symbol |
| `1.4.2` | **Übung spontan hinzufügen, auch mitten im Training.** *(Zuerst als `1.5.0` gebaut und vom Benutzer zurückgenommen - „Kein Versionssprung auf eine höhere Basisnummer außer wenn ich es sage." Das Paket hatte den Rechner nie verlassen, die Nummer wurde wieder frei; seither steht die Regel unter* Zählweise *in `CLAUDE.md`.)* Wunsch des Benutzers am 2026-08-30: *„Manchmal habe ich im Studio noch Lust eine weitere Übung zu machen und würde die dann gerne mit protokollieren können. Evtl. auch hier mit der Abfrage ob man diese permanent oder nur für dieses Training hinzufügen möchte."* Der dauerhafte Teil war klein — `add_exercise` gab es längst, es fehlte der Weg dorthin. Der zweite Teil war die eigentliche Arbeit, und die Wahl zwischen beiden wurde vor dem Bauen ausdrücklich gestellt: „nur diese Einheit" kostet eine Spalte und den Filter an einem Dutzend Stellen, weil `plan_exercises` bis dahin schlicht *der Plan* war. Der Benutzer entschied sich für beide Varianten. **Drei Entwürfe wurden verworfen, bevor `plan_exercises.session_id` blieb:** eine eigene Tabelle für Tagesübungen scheitert daran, dass `workout_log` über `plan_exercise_id` an einer echten Planposition hängt (Fallstrick 4) — ohne sie wäre der Protokolleintrag nicht zuzuordnen und „x/n" nicht zählbar. Die Zeile am Ende der Einheit zu **löschen** scheitert an derselben Stelle von der anderen Seite: `ON DELETE SET NULL` leerte `plan_exercise_id`, und der Verlauf verlöre die Zuordnung — lautlos. Bleibt: Die Zeile bleibt stehen und wird ausgeblendet, wo der *Plan* gemeint ist. **Acht tragende Stellen** mussten das lernen (Trainingsansicht, beide „x/n"-Zählungen, Planverwaltung, Kopieren, Zurücksetzen, Fingerabdruck, Textexport) und vier weitere der Ordnung halber. **Die Struktursperre wurde für das Hinzufügen gelöst** — als einzige der drei Strukturaktionen: Ans Ende zu hängen verschiebt keine bestehende Position, und dass „n" wächst, hat der Benutzer selbst ausgelöst. Umsortieren und Entfernen bleiben gesperrt. Im selben Zug wanderte die **Übungsauswahl aus `plans.js` nach `assets/app.js`** und in ein Partial: Sie wird jetzt von zwei Seiten gebraucht, und der Kommentar „inline und kein Partial, sie wird nur hier gebraucht" stimmte nicht mehr. **Die schärfste Gegenprobe war das Zurücksetzen auf die Vorlage:** Ohne den Filter dort *verschwindet* die Tagesposition — der Reset löscht sie, weil sie in der Vorlage nicht vorkommt, und nimmt dem Protokolleintrag die Zuordnung mit. Genau der Fehler, gegen den die ganze Konstruktion gebaut ist. Gefunden wurden die offenen Stellen mit einer Prüfung **je Funktion** statt je Zeile — die Filter stehen mal in der WHERE-Klausel, mal als Riegel zwanzig Zeilen darüber; sie meldete vier ungeklärte Stellen, davon eine echte Lücke: `remove_exercise` hätte eine Tagesposition gelöscht |
| `1.4.1` | **Das Anlege-Formular nach der ersten Sicht auf `1.4.0`.** Vier Ansagen des Benutzers am 2026-08-30, unmittelbar nachdem die Version draußen war: *„beim Anlegen einer Übung sollte bei der Auswahl von ‚Ausdauer – Distanz und Zeit' nicht eine Muskelgruppe zwingend ausgewählt werden müssen. Vielmehr würde ich sagen Trainingsgerät und ‚Erfassung' sollten vor die Muskelgruppen wandern und wenn man bei ‚Erfassung' Ausdauer und nicht Kraft auswählt, sollte man den großen Block mit den Muskelgruppen ausblenden. Zudem ‚Erfassung' in ‚Trainingsart' umbenennen."* Alle vier umgesetzt. Die Umbenennung blieb dabei **auf die Oberfläche beschränkt** — die Spalte heißt weiter `erfassung`, nach demselben Muster wie bei `GERAETE`: Der Schlüssel steht in der Datenbank, die Beschriftung nur im Code. Ein `RENAME COLUMN` wäre eine Migration für ein Label gewesen. Dass Gerät und Trainingsart **vor** die Muskelgruppen wandern, ist zugleich die technische Voraussetzung für Punkt drei: Ein Auswahlfeld, das den Block über sich ausblendet, ließe beim Umschalten das halbe Formular nach oben springen. Der Block wird ausgeblendet **und** deaktiviert, weil `new FormData(formular)` auch unsichtbare Felder einsammelt — ohne das `disabled` reiste eine vorher angehakte Gruppe mit, die niemand mehr sehen kann. **Die eigentliche Arbeit steckte in einer Folge, die niemand angesprochen hatte:** Ohne Muskelgruppe liefert `primaergruppe_von_uebung()` für eine Ausdauerübung `null`, und `tausch_vorschlaege()` stieg genau dort mit einer leeren Liste aus — die Ausdauerübungen hätten ihren Tauschpartner verloren, und zwar lautlos. Seither hat die Funktion einen eigenen Zweig: Bei Ausdauer entscheidet allein die Trainingsart, vorgeschlagen werden alle anderen Ausdauerübungen, alphabetisch und ohne die Rangfolge „nächstliegende Untergruppe zuerst" — eine nähere oder fernere Ausdauerform gibt es nicht. Damit trägt der Tausch genau den Fall, für den es ihn gibt: Das Laufband ist besetzt, also nimmt man den Crosstrainer. **Beim Prüfen noch einmal dieselbe Falle wie am Vortag, diesmal in der JS-Attrappe:** Sie startete im *erwarteten* Endzustand, und die Gegenprobe fiel deshalb an der falschen Stelle durch — der Test maß einen Wert, der schon vorher stimmte. Erst mit einem Startzustand, der dem Erwarteten entgegengesetzt ist, fielen beide Gegenproben genau dort durch, wo sie sollten |
| `1.4.0` | **Ausdauergeräte: Laufband, Crosstrainer, Rudergerät.** Angestoßen vom Benutzer am 2026-08-30: *„Ziel ist es auch Cardio Geräte zu integrieren … Für diese Geräte bräuchten wir dann natürlich nicht Wiederholungen und Gewicht sondern Distanz (in Metern) und Zeit (in Minuten). Im Verlauf soll man bei diesen Geräten dann zudem eine Pace ausgewiesen bekommen."* Drei neue Gerätetypen — und, wichtiger, eine zweite und davon **unabhängige** Eigenschaft der Übung: `exercises.erfassung`, `kraft` oder `ausdauer`. **Woran sie hängt, war die eigentliche Entscheidung.** Die naheliegende Abkürzung wäre gewesen, sie aus `equipment` abzuleiten — Laufband heißt Ausdauer, keine neue Spalte, kein Formularfeld. Der Benutzer hat den Fall selbst umgedreht: *„Von der Logik wäre es sinnvoller es an die Übung zu binden und nicht ans Gerät, oder? Also wenn man laufen will, ist das cardio, egal ob im Freien oder auf dem Laufband."* Die Übung ist die Tätigkeit und bestimmt, was gemessen wird; das Gerät sagt nur, wo. Drei Dinge, die eine Ableitung nicht geleistet hätte: eine spätere Ausdauerform ohne eigenes Gerät (Seilspringen, Laufen im Freien) hätte einen erfundenen Gerätetyp in einer Liste gebraucht, die die Frage „Maschine oder Kurzhantel?" beantwortet; `tausch_vorschlaege()` hätte entgegen der ausgeschriebenen Regel aus §7.5 doch nach dem Gerät filtern müssen; und eine spätere Zeile in `GERAETE` hätte **rückwirkend die Bedeutung protokollierter Werte gekippt** — lautlos, mit `ok:true`. Weiter entschieden: mehrere Intervalle statt eines Feldpaars je Einheit (der bestehende Satzblock trägt sie, nur mit dem anderen Feldpaar), Zeiteingabe als **`mm:ss`** statt Dezimalminuten, und die Pace **auch** in der Trainingsansicht, nicht nur im Verlauf — sonst sieht man beim Eintragen nicht, ob man schneller war als letztes Mal. Fünf rein additive Spalten; `distanz_m`/`dauer_s` stehen an `workout_log` **und** an `workout_sets`, aus genau dem Grund, aus dem `weight` an beiden steht: Im einfachen Modus gibt es keine Satzzeilen, und der Verlauf liest nur `workout_log`. **Anders als `weight` sind sie Summen und kein Maximum** — zwei Intervalle zu 1000 m sind 2000 gelaufene Meter, zwei Sätze zu 40 kg keine 80. Der Warteschlangen-Schlüssel blieb `-v3`: Die Form eines Eintrags ändert sich nicht, und ein Sprung hätte nur die Eingaben von jedem verworfen, der gerade trainiert. **Beim Prüfen zwei Löcher im eigenen Prüfstand**, beide vom selben Typ wie die in `CLAUDE.md` gesammelten: Der Dev-Server lief mit `opcache.revalidate_freq = 2` und antwortete nach dem Entfernen einer Zeile noch aus dem alten Bytecode — die Gegenprobe bestätigte damit eine Regel, die gar nicht mehr lief. Und die erste Tausch-Gegenprobe kam für beide Richtungen leer zurück, weil alle Übungen im selben Plan standen und der Ausschluss „steht ohnehin im Plan" vor dem Erfassungsfilter greift; das sah nach einem Befund aus und war keiner. Beides steht jetzt unter *Lokale Entwicklung* |
| `1.3.2` | **Eine Splitkarte statt aller, Auswahlfeld im Kopf, Umbenennen im Dialog.** Drei Ansagen des Benutzers vom 2026-08-28, nachdem `1.3.1` live war: die Admin-Kacheln in der Reihenfolge Vorlagen – Übungen – Muskelgruppen – Benutzer – Wartung; auf `splits.php` „nicht alle gleichzeitig … sondern zuerst nur der aktive Split“, im Kartenkopf ein Pulldown zum Wechseln; und *Umbenennen* öffnet folglich „ein überblendetes Eingabefeld“. Dasselbe Schema auf `admin_splits.php`. Gerendert werden weiterhin **alle** Karten, die übrigen mit `[hidden]` — der Wechsel ist damit ein Umschalten im Browser statt eines Seitenaufbaus und funktioniert im Studio ohne Verbindung; die Daten stehen ohnehin schon in der Seite. Das Auswahlfeld steht bewusst in **jeder** Karte mit jeweils derselben Liste: Die sichtbare trägt dann immer schon den richtigen Eintrag, und ein einzelnes Feld über der Liste bräuchte einen Gleichlauf, der auseinanderläuft. `?split=` per `replaceState`, weil diese Seiten nach jeder Änderung neu laden — ohne den Parameter landete man wieder beim aktiven statt bei dem, den man bearbeitet hat. Der Umbenennen-Dialog geht **nicht** über `splitAktion()`: Ein 422 gehört unter das Feld, in dem der zu lange Name steht, nicht an die Karte hinter dem Dialog |
| `1.3.1` | **Dieselbe Auslieferung wie `1.2.23`, unter neuer Nummer.** Der Benutzer hat den Umbau am 2026-08-28 als `1.3.1` ausgerollt statt als `1.2.23` — die neue Adminseite und das geänderte Handling waren ihm den Sprung auf eine neue Minor wert. **Die `VERSION`-Datei im Image sagt aber weiterhin `1.2.23`** (gemessen: die laufende Instanz liefert `app.js?v=1.2.23`), das Image trägt nur den Tag `1.3.1`. Damit ist `1.3.1` eine ausgelieferte Nummer, die nie in `VERSION` gestanden hat; in der Datei folgt auf `1.2.23` direkt `1.3.2`, und die Lücke ist keine übersprungene Version, sondern genau dieser Fall |
| `1.2.23` | **Die Splits-Seite aufgeräumt: Verwaltung in den Adminbereich.** Angestoßen vom Benutzer am 2026-08-28: *„Die User Splits und Vorlagen sollten meiner Meinung nach in den Adminbereich wandern und dort auf einer eigenen Seite zusammengefasst werden … So dass unter Split im Hauptmenü für alle (egal ob user oder admins) nur die eigenen Splits stehen bzw. die Möglichkeit vorhanden ist, sich Vorlagen zu sich selbst zu kopieren.“* Bis `1.2.22` trug `splits.php` drei Aufgaben zugleich — eigener Bestand, *User Splits* (nur Admin) und der Katalog mit Umbenennen, Bearbeiten, Duplizieren und Löschen daran. Für einen normalen Benutzer war die halbe Seite ein Katalog mit genau einem erlaubten Knopf darin; für einen Admin war dieselbe Seite gleichzeitig Selbstbedienung und Verwaltung, und `split_karte()` bildete beides in einer Funktion mit vier Fallunterscheidungen ab. Jetzt: **`splits.php` zeigt nur den eigenen Bestand, für jeden gleich**, dazu den Kasten *Vorlage übernehmen* (Auswahlfeld, Planvorschau, *Zu mir kopieren*, *Als Text*) — baugleich zum Kandidaten-Kasten, den es schon gab. **`admin_splits.php` („Vorlagen“, fünfte Admin-Kachel)** trägt den Katalog, *Vorlage anlegen*, *Aus einem Benutzer-Split* und *Splits anderer Benutzer*. Drei Entscheidungen des Benutzers legten den Zuschnitt fest: der Katalog verschwindet **ganz** von `splits.php`; `plans.php` führt im Auswahlfeld **nur noch eigene Splits**; und kein *Zu mir kopieren* im Adminbereich — ein Admin holt sich eine Vorlage denselben Weg wie jeder andere. **Zwei Dinge fielen beim Umbau auf, die der Plan nicht vorhergesehen hatte.** (a) Die Rückfrage vor *Auf Vorlage zurücksetzen* las ihre Vorschau bis dahin **an der Vorlagenkarte derselben Seite** ab — die steht dort nicht mehr; der Wert reist jetzt als `data-plaene` an der `<option>` des Herkunftsfelds mit, weiterhin server-gerendert. (b) Mit dem Auswahlfeld in `plans.php` verschwand der **einzige** Weg zum Split eines anderen Benutzers (§6.4 erlaubt ihn ausdrücklich); daraus wurde der dritte Abschnitt auf `admin_splits.php` — bewusst **ungefiltert** und neben dem Veröffentlichen-Pulldown, weil die beiden verschiedene Fragen stellen: was sich veröffentlichen *lässt* gegen was es *gibt*. **Der IDOR-Schutz in `plans.php` ist damit nicht mehr allein die Liste**, sondern Liste plus eine Prüfung: Ein `?split=` darüber hinaus läuft gegen `split_darf_bearbeiten()` — die Regel aus `split_zugriff_api()`, jetzt als Prädikat, weil zwei Aufrufer dasselbe Nein verschieden beantworten (403 gegen stillen Rückfall). `api/splits.php` blieb bis auf das neue Feld `namen` an `reset` **unverändert**, ebenso Schema und Daten: Es wanderte Oberfläche. **Im selben Zug der zweite Wunsch desselben Tages:** *„Wenn ich bei einem kopierten Split die Namen der Pläne ändere, möchte ich, dass diese Namensänderung allein noch nicht das Event auslöst, den Reset auf die Vorlage anzubieten.“* Bis dahin trug der Abgleich die Plannamen im Fingerabdruck (`split_abgleich_signaturen()`) — eine eigene Beschriftung ließ den Knopf erscheinen, der ebenjene Beschriftung wieder weggenommen hätte. Jetzt entscheidet allein der Inhalt, und die Namen sind eine zweite Frage: `split_zuruecksetzen($id, $namen)`, erfragt über einen `<dialog>` mit Kästchen statt des bisherigen `window.confirm` — denn eine zweiteilige Frage passt nicht in ein `confirm`, und zwei hintereinander wären zwei Klicks für eine Entscheidung. Das Kästchen erscheint nur bei wirklich abweichenden Namen (`namen_weichen_ab`), ist stets leer vorbelegt, und ein verborgenes Häkchen zählt nicht. Fehlt `namen` in der Nutzlast, wird **nicht** umbenannt — die umgekehrte Vorgabe zu `done` in Fallstrick 18, weil ein nicht geäußerter Wunsch hier stiller Datenverlust wäre |
