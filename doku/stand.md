...
*Letzte Aktualisierung: 2026-09-22 (`1.5.4` live nachgemessen und als `claude` geprüft)*
...
| **Arbeitsstand** | `1.5.4` — **gebaut am 2026-09-22 auf Ansage**, `deploy/trainingsplan-build-1.5.4.tar.gz`; **der Image-Bau in Portainer lief durch** (Auskunft des Benutzers, 2026-09-22) — damit ist die AVIF-Vermutung gestützt, aber nicht bewiesen. **Live seit 2026-09-22**, siehe oben. `1.5.3` wurde am 2026-09-22 gebaut und hochgeladen, **der Image-Bau in Portainer blieb aber zweimal endlos hängen**; live läuft weiter `1.5.2` (nachgemessen am 2026-09-22). Vermutete Ursache: `libavif-dev`/`--with-avif` im `Dockerfile`. `1.5.4` ist `1.5.3` **ohne AVIF**: Die `RUN`-Zeile ist wieder die von `1.5.2`, GIF bleibt (braucht nichts am Image). `1.5.2` ist committet und gepusht als `a8e59b2`. Der übrige Inhalt von `1.5.3` bleibt: Beim Bildausschnitt steht „ganzes Bild“ als erster Eintrag und ist für **neue** Übungen vorausgewählt (`ZUSCHNITT_VORGABE = 'ganz'`); bestehende Übungen behalten ihren gespeicherten Wert, die Spalte behält `DEFAULT 'mitte'`. Lokal am Formular geprüft. **Dazu GIF als Uploadformat** (AVIF entfallen, s. o.) (`lib/upload.php`, `accept` im Formular); **das `Dockerfile` ändert sich** (`libavif-dev`, `--with-avif`), der Image-Bau dauert damit etwas länger. Lokal gegen PHP 8.5 von der Dekodierung an geprüft (ohne `fileinfo`, der Typtest selbst also nicht); **AVIF im Container erst nach dem Einspielen prüfbar**. **Dazu die Zeile „zuletzt …“ in der Trainingsansicht zweizeilig** („zuletzt 3 Sätze am 20.09.26“, darunter die Satzliste); lokal am gerenderten HTML geprüft, **die Darstellung am Handy steht aus**. Davor `1.5.2` — gebaut am 2026-09-21 auf Ansage. Neue Nummer, weil `1.5.1` live läuft (nachgemessen am 2026-09-21, `app.js?v=1.5.1`). Inhalt: Nach dem Speichern einer bearbeiteten Übung springt `admin_exercises.php` zu genau dieser Übung; die Muskelgruppen einer Übung stehen überall in der neuen Reihenfolge (`MG_ZUORDNUNG_ORDER` in `lib/muskelgruppen.php`: primär, dann dieselbe Hauptgruppe, dann wie auf `admin_muscle_groups.php`). Reihenfolge lokal an allen fünf Abfragen samt Gegenprobe geprüft; **der Sprung ist nur im Browser prüfbar und steht aus.** Davor `1.5.1` — **gebaut am 2026-09-17 auf Ansage**, `deploy/trainingsplan-build-1.5.1.tar.gz`; Sprung auf `1.5` auf Ansage des Benutzers (`1.4.13` wurde nie gebaut, `1.5.0` gibt es nicht). Vom Benutzer nach dem Einspielen getestet, ohne Befund. **Committet und gepusht** am 2026-09-17 (`eb9718d`, zusammen mit `1.4.11` und `1.4.12`). Ausdauerzeit in Eingabe und Trainingsansicht nur noch als Minuten und Sekunden (`73:25`), Eingabe `h:mm:ss` entfällt; der Verlauf zeigt ab einer Stunde `1:13:25`. Dazu **Einheiten nachträglich bearbeiten** (Sätze ändern/nachtragen/entfernen, keine Übungen; `api/log.php → correct`, Fallstrick 35). **Darstellung am Handy nicht geprüft**, ebenso wenig die echte Sicherheitsabfrage im Browser (nur unter jsdom). Kein Schema. **Darstellung nicht geprüft.** Davor: `1.4.12` — **gebaut am 2026-09-15 auf Ansage**, am 2026-09-17 **live nachgemessen** (`app.js?v=1.4.12`), `deploy/trainingsplan-build-1.4.12.tar.gz`. Nach der Regel *gebaut heißt ausgerollt* damit draußen; **die Live-Nummer ist nicht nachgemessen**. **Noch nicht committet** (`1.4.11` ebenfalls nicht). Neu: Bildausschnitt *ganzes Bild* (breite Bilder im Vorschaurahmen verkleinert statt beschnitten, große Ansicht unverändert). Kein Schema. **Darstellung nicht geprüft.** Davor: `1.4.11` — **gebaut am 2026-09-15 auf Ansage**, `deploy/trainingsplan-build-1.4.11.tar.gz`. Nach der Regel *gebaut heißt ausgerollt* damit draußen; **die Live-Nummer ist nicht nachgemessen**. **Noch nicht committet.** Neu: Übungen mit Unterstützung („Gewicht bedeutet: Unterstützung — weniger ist besser“, `exercises.gewicht_wirkung`, Fallstrick 34). **Schema: eine neue Spalte, rein additiv** — auf der Live-DB bekommt jede Übung `last`, am Verhalten ändert sich nichts, bis jemand eine Übung umstellt. Umstellen rechnet die Leitgewichte **dieser einen Übung** aus ihren Sätzen neu. Lokal per `curl` geprüft samt drei Gegenproben (`doku/historie.md`). **Nicht geprüft: die Darstellung** (Abzeichen, Formularfeld, gespiegelte Kurve am Handy). Davor: `1.4.10` — **gebaut am 2026-09-14 auf Ansage**, `deploy/trainingsplan-build-1.4.10.tar.gz`. **Live nachgemessen und geprüft am 2026-09-14** (siehe *Live*). **Committet und gepusht** am 2026-09-14 als ein Commit „Version 1.4.8 bis 1.4.10“ (`1.4.8`–`1.4.10` gemeinsam; die Kennung steht in `git log`). Eine Korrektur, gemeldet am 2026-09-14: Der Muskelgruppen-Filter im Verlauf lieferte bei „Brust“ auch die Abschnitte „Schultern“ und „Arme“, weil er — wie die Übungsauswahl — jede Zuordnung zählte. Er prüft jetzt die Primärgruppe (`pgrp` aus `MG_SORT_JOIN`). **Lokal nachgestellt und behoben**, mit Nebengruppen, die in eine andere Hauptgruppe zeigen: Für jede Muskelgruppe erscheint nur noch ihre eigene Hauptgruppen-Überschrift. Zwei Gegenproben (Untergruppen-Einschluss entfernt / alte 1.4.9-Regel) kippten genau die erwarteten Gruppen. **Warum `1.4.9` durch die Prüfung kam:** Die einzige Nebengruppe im damaligen Prüfbestand (Trizeps an Bankdrücken) zeigte in keine gewählte Gruppe, deren Ergebnis ich mir angesehen und beanstandet hätte — ich hatte die weite Regel ausdrücklich so gewollt. Davor: `1.4.9` — **gebaut am 2026-09-14 auf Ansage**, `deploy/trainingsplan-build-1.4.9.tar.gz`. Nach der Regel *gebaut heißt ausgerollt* damit draußen; **die Live-Nummer ist nicht nachgemessen**. **Noch nicht committet.** Eine Änderung (Wunsch vom 2026-09-14): In `history.php` erscheint bei *Übungen* + *alle Einheiten* + *Nach Muskelgruppe* ein drittes Auswahlfeld **Muskelgruppe**, aufgebaut und filternd wie in der Übungsauswahl (jede Zuordnung zählt, Hauptgruppe schließt Untergruppen ein; §7.8). **Lokal per `curl` geprüft** (Hauptgruppe, Untergruppe, Nebengruppe, leere Gruppe, unbekannte ID, Feld und Filter entfallen bei anderer Sortierung oder gewählter Einheit), zwei Gegenproben (Untergruppen-Einschluss entfernt / Filter unabhängig vom Zustand) kippten genau die erwarteten Fälle; `history.js` unter Node ausgeführt, samt zwei Gegenproben. **Darstellung nicht geprüft.** Davor: `1.4.8` — **gebaut am 2026-09-14 auf Ansage**, `deploy/trainingsplan-build-1.4.8.tar.gz`. Nach der Regel *gebaut heißt ausgerollt* damit draußen; **die Live-Nummer ist nicht nachgemessen**. **Noch nicht committet.** Änderungen am Verlauf (`history.php`, Rückmeldung vom 2026-09-14): Übungsname in einer Einheit führt in die Einzelansicht der Übung (alle Einheiten, die Herkunftseinheit hinterlegt), das Datum dort zurück zur Einheit; die Übungsansicht hat zwei Auswahlfelder — **Einheit** (grenzt auf deren Übungen ein) und **Sortierung** (zuletzt / am häufigsten / nach Muskelgruppe / Reihenfolge im Training). Beschrieben in `LASTENHEFT.md` §7.8. **Lokal per `curl` gegen PHP 8.5 geprüft**, mit zwei Splits, Unter- und Hauptgruppen, einer Ausdauerübung und einem zweiten Benutzer: alle Sortierungen, Eingrenzung, Einzelansicht, 60er-Kappung mit „alle anzeigen“, Datums-Links; die Einheit eines anderen Benutzers fällt still auf „alle Einheiten“ zurück. Vier Gegenproben in `uebungen_mit_verlauf()` (Einheitsfilter / `user_id` in der Unterabfrage / Trainingsreihenfolge / Häufigkeit entfernt) kippten je genau die erwartete Liste. Der Change-Handler in `history.js` unter Node ausgeführt, samt Gegenprobe. **Nicht geprüft: die Darstellung** — Filterleiste und Gruppenüberschriften am Handy, wie lang die Einheiten-Einträge im aufgeklappten Auswahlfeld umbrechen. Davor: `1.4.7` — **gebaut am 2026-09-03 auf Ansage**, `deploy/trainingsplan-build-1.4.7.tar.gz`. Nach der Regel *gebaut heißt ausgerollt* damit draußen; **die Live-Nummer ist nicht nachgemessen** — wer sie benutzt, misst vorher mit dem `curl`-Einzeiler unten. **Committet und gepusht** als `a421b45`. Zwei Änderungen, beide am 2026-09-03. **Erstens:** Die Reihenfolge der Muskelgruppen ordnet jetzt auch die Übungen — Übungsverwaltung und Übungsauswahl folgen ihr statt dem Alphabet (`lib/muskelgruppen.php`, Fallstrick 9). Ansage des Benutzers: „Wenn ich Latissimus unter Trapez verschiebe, sollen alle Übungen in der Reihenfolge ausgegeben werden." **Am Bestand nachgemessen** — Gesamtliste, auf eine Hauptgruppe eingeschränkt, mit Gerätefilter, mit einer archivierten Übung, und über die echte `reorder`-Aktion: Nach dem Verschieben dreht sich die Übungsreihenfolge in beiden Listen mit. Drei Gegenproben (Gruppenordnung raus / `LEFT JOIN` auf inneren Verbund / Wurzel-Stufe raus) fielen genau erwartungsgemäß aus; die mittlere ließ die Ausdauerübung ohne Muskelgruppe lautlos verschwinden. **Zweitens:** Die Sortierpfeile auf `admin_muscle_groups.php` funktionieren wieder. Gemeldet am 2026-09-03 aus dem Live-System („bei den Untergruppen tun die Pfeile nichts"). Ursache war, dass `insertBefore()` immer gegen `#gruppen-liste` einfügte, während eine Untergruppe in ihrem eigenen `<ul>` steht — der Aufruf warf, und niemand sah es. **Am Browser reproduziert und nach dem Fix gegengeprüft**, samt drei gezielten Rücknahmen (Halter nicht überspringen / Halter nicht mitnehmen / wieder gegen die oberste Liste einfügen): Jedes Mal fiel genau das Erwartete durch und nur das. Mitgeändert: Eine Hauptgruppe wandert jetzt samt ihren Untergruppen, Randpfeile sind gesperrt, ein gescheitertes Speichern nimmt die Verschiebung zurück statt neu zu laden. **Noch nicht committet.** Davor: `1.4.6` — gebaut am 2026-09-02 auf Ansage, `deploy/trainingsplan-build-1.4.6.tar.gz`. Nach der Regel *gebaut heißt ausgerollt* damit draußen; **die Live-Nummer ist nicht nachgemessen** — wer sie benutzt, misst vorher mit dem `curl`-Einzeiler unten. **Noch nicht committet.** Eine Änderung: kein `autofocus` mehr auf `login.php` und `password.php`; der Fokus wird in `login.js` / `password.js` gesetzt und nur am Zeigegerät (`matchMedia('(pointer: coarse)')`). Grund ist eine Rückmeldung vom 2026-09-02 — auf einem Pixel 10 bot Proton Pass in Firefox die Zugangsdaten nicht an, und zwar ohne dass auch nur das Symbol über der Tastatur erschien: Firefox auf Android meldet ein beim Laden schon fokussiertes Feld nicht an Androids Autofill-Dienst. Die Regel steht in `CLAUDE.md` unter *Frontend*. **Die Wirkung ist NICHT nachgewiesen** und nur am Gerät prüfbar; belegt ist allein, dass der Fokus am Zeigegerät gesetzt wird und am Touchgerät nicht (unter Node ausgeführt, samt Gegenprobe). Zuletzt committet: `4583b14` mit `1.4.5`. |
| **Rollback-Ziel** | **`trainingsplan:1.4.8`** — dasselbe Schema; es fehlt nur das Muskelgruppen-Feld im Verlauf. **Nicht `1.4.9`**: Dort ist der Filter fehlerhaft (Nebengruppen). Davor: **`trainingsplan:1.4.6`**, falls das Image noch in Portainer steht — ein Schritt zurück, **dasselbe Schema**, seit dem 2026-09-02 im Studio in Benutzung. Es verliert allein die beiden Änderungen aus `1.4.7`: Die Sortierpfeile bei den Untergruppen tun wieder nichts, und die Übungslisten stehen wieder alphabetisch statt in der Reihenfolge der Muskelgruppen. **Beides sind Bedien- und Anzeigefehler, keine Datenfehler** — anders als beim Schritt hinter `1.4.5`. Davor: **`trainingsplan:1.4.4`** — auch ein Schritt zurück, **dasselbe Schema**, und es lief zwei Tage im Studio. Es verliert allein die beiden Korrekturen aus `1.4.5`: Das Rennen in der Warteschlange kommt zurück (ein Häkchen kann wieder verlorengehen), und beim Beenden wird nichts mehr nachgetragen. **Beides sind Datenfehler und keine Anzeigefehler** — anders als bei früheren Rollback-Zielen wäre dieser Schritt zurück also nicht folgenlos. **Tiefer zurück nur im Notfall:** `1.2.11` war lange das Ziel und ist durchgeprüft, kostet heute aber die Ausdauergeräte — die Spalten sind additiv, die Daten überleben, aber `1.2.11` kennt `erfassung` nicht und zeigte eine Laufband-Übung als Kraftübung. Der alte Vorbehalt gegen `1.2.15`–`1.2.21` ist hinfällig: Diese Stände sind seit Wochen in täglicher Benutzung, die offenen Punkte 0/0b/0c betreffen nur die Beurteilung am Gerät. |
...
| Trainingsgeräte | Benutzt werden Maschine, Kabelzug, Kurzhantel und Körpergewicht. **Nur Kettlebell ist bisher unbenutzt** — Multipresse und Langhantel sind in Gebrauch (am Live-Bestand nachgesehen am 2026-09-22) — ein Filter darauf läuft leer, das ist kein Fehler. Seit `1.4.0` gibt es zusätzlich **Laufband, Crosstrainer und Rudergerät** (am 2026-08-30 hat der Benutzer *Laufen* und *Crosstraining* angelegt, das Rudergerät ist noch unbenutzt), seit `1.4.3` **Stairmaster** |
| Übungstexte | **Jede Kraftübung trägt Ausführung UND Beschreibung** (Stand 2026-09-22). 15 fehlende Beschreibungen hat Claude auf Wunsch des Benutzers über `api/exercises.php → update` nachgetragen — mit vollem Feldsatz aus einem vorher gezogenen Abzug (Fallstrick 22), danach alle 76 Übungen feldweise verglichen: nur diese 15 Beschreibungen geändert, Gruppen, Bilder und Zuschnitt unverändert. Danach der Tippfehler „Burst“ → „Brust“ in der Ausführung von #47 korrigiert, ebenso feldweise gegengeprüft |
