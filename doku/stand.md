...
*Letzte Aktualisierung: 2026-09-01 (nach dem Rollout von `1.4.5`)*
...
| **Arbeitsstand** | `1.4.5` — **deckungsgleich mit Live**, nichts Ungebautes offen. **Noch nicht committet.** Zwei Dinge, beide zum verschwundenen Häkchen: **(1)** die zweite Hälfte des Fehlers aus Fallstrick 13 — `abarbeiten()` in `index.js` löschte einen Warteschlangen-Eintrag, der während des laufenden Aufrufs neu gesetzt wurde (`eintragAbschliessen()`, siehe *Offen*, Punkt 0w). **(2)** die Sicherheitsstufe darunter auf Ansage des Benutzers: Beim Beenden bekommt jede Position mit mindestens einem Satz `done = 1` (`protokollierte_positionen_abschliessen()` in `lib/training.php`, §7.6, Fallstrick 18). Beides nachgewiesen am ausgeführten Code samt Gegenprobe — (1) unter Node, (2) gegen eine Testdatenbank, über HTTP und **am Live-Container** (siehe *Live*). **Nicht** am Gerät geprüft ist die Bedienung: dass der Haken beim Tippen im Studio auch wirklich stehen bleibt. Zuletzt committet und gepusht: `a7c1ecd` mit `1.4.3`/`1.4.4`. |
